...
app.get('/menu/search', async (req, res) => {
    try {
        ...
        else if (stype == "category") query = "select A.id as id, A.name as name from dishes as A, category as B where A.category_id = B.id and lower(B.name) like '%"+ input.toLowerCase() + "%' order by A.name;";
...
app.get('/menu/food/:f', async (req, res) => {
    try {
        ...
        const category = await mysql.query("select name from category where id = ?", foods[0][0].category_id);
        const origin = await mysql.query("select group_concat(origin.name) as name from createdFrom, dishes, origin where createdFrom.dish_id = dishes.id and createdFrom.origin_id = origin.id and dishes.id = ?", req.params.f);
        ...
        res.render('pages/dishInfo', {foods: foods[0], origin: origin[0], ingredient: ingredient[0], category: category[0]});
    }
    catch (err) {
        res.render('error/cantConnect');
    }
